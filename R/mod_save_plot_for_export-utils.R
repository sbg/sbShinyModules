...
#' Generate random ID
#'
#' This function generates a random ID of the specified number of bytes,
...
check_filename <- function(filename, device) {
  # Common extensions
  allowed_extensions <- c("png", "pdf", "svg", "jpeg", "bmp", "tiff")
...
#'  path, along with its subdirectories with expected names 'project-files' and
...
check_sbg_directory_path <- function(sbg_path) {
  ...
  if (!dir.exists(file.path(sbg_path, "project-files")) ||
    ...
    shinyalert::shinyalert(
      ...
      text = "The provided directory path doesn't contain
      project-files and/or output-files sub-directories.",
      type = "error"
    )
  }
}
...
#'  look for the existence of the provided file.
